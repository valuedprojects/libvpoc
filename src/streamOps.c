...
int StreamRead (int streamfd, unsigned char * Buffer, unsigned int Length, char * Description)
{
...
int StreamReadDelimited (int streamfd, unsigned char * Buffer, unsigned int Length, unsigned char Delimiter, char * Description)
{
  ...
  while (position < (Length-1))
	{
	  if ( (status = StreamRead (streamfd, Buffer+position, 1, Description)) != 1 )
		{

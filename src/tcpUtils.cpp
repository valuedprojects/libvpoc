...
int tcp_Server::tcp_bind_setup_fd (int fd, sockaddr * pGenericSocketAddress)
{
	socklen_t addrlen;
	// Find size of socket:
 ...
 */
int tcp_Server::tcp_PF_UNIX_srv_open (char * pathName)
{
	int fd;
 ...
 */
int tcp_Server::tcp_PF_INET_srv_open (char * service, int port)
{
	int fd;
 ...
 */
void tcp_Server::handleExitedServerChildren (int signum)
{
	pid_t pid;
	int childStatus;
 ...
 */
void tcp_Server::handleExitServer (int signum)
{
...
int tcp_Server::serverProcessing(int read_fd, int write_fd)
{
	int charCount = 30;
	char C;
	int status;
...
tcp_Server::tcp_Server(char* service, int port)
{
	...
	// Open the tcp server socket...
	server_fd = tcp_PF_INET_srv_open (service, port);
	if ( server_fd == -1 )
	{
		// Something bad happened when setting up the TCP socket.
		cout << "Problem setting up server socket" << endl;
		...
		while (!accept_one)
		{
			if ( (connection_fd=accept(server_fd, &peerAddr, &peerAddrLength)) < 0 )
			{
				...
				else if (sigstate == TERMINTERRUPTION)
				{
					...
					/*
					 * Find out what type of socket we're dealing with.
					 * If it's an AF_UNIX socket, we need to "unlink" (aka remove)
					 * the file node so that we can connect up again.
					 */
					// Enough space for a good long pathname...
					unsigned char buffer[512];
					struct sockaddr * pSockAddr = (struct sockaddr *)buffer;
					...
					close(tcp_Server::server_fd);

					if (pSockAddr->sa_family == AF_UNIX)
					{
						// Map to sockaddr_un, grab the pathname so that it can be unlinked.
						struct sockaddr_un * pSockAddrUn = (struct sockaddr_un*)buffer;
						cout << "pSockAddrUn->sun_path: " << pSockAddrUn->sun_path << endl;
						unlink(pSockAddrUn->sun_path);
					}
					exit(0);
				}
				else
...
int tcp_Server::server_fd = 0;
int tcp_Server::sigstate = 0;
